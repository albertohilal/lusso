...
class ResalesOnlineConnector {
    ...
    private function get_demo_data($params) {
        ...
        // Filtrar datos de demostración según los parámetros
        if (!empty($params['agency_filter'])) {
            switch ($params['agency_filter']) {
                case 'destacados':
                    // Devolver solo las 3 primeras propiedades para destacados
                    $demo_data['Properties'] = array_slice($demo_data['Properties'], 0, 3);
                    break;
                case 'alquiler_corto':
                    // Para alquiler corto, mostrar propiedades más pequeñas
                    $demo_data['Properties'] = array_filter($demo_data['Properties'], function($prop) {
                        return $prop['PropertyType'] === 'Apartment' || $prop['PropertyType'] === 'Bungalow';
                    });
                    break;
                case 'alquiler_largo':
                    // Para alquiler largo, mostrar casas familiares
                    $demo_data['Properties'] = array_filter($demo_data['Properties'], function($prop) {
                        return $prop['PropertyType'] === 'Townhouse' || $prop['Bedrooms'] >= 3;
                    });
                    break;
            }
        }
        
        // Aplicar límite de página si se especifica
        if (isset($params['P_PageSize'])) {
            $limit = intval($params['P_PageSize']);
            $demo_data['Properties'] = array_slice($demo_data['Properties'], 0, $limit);
        }
        
        // Filtrar por tipo de propiedad si se especifica
        if (!empty($params['P_PropertyTypes'])) {
            $type = $params['P_PropertyTypes'];
            $demo_data['Properties'] = array_filter($demo_data['Properties'], function($prop) use ($type) {
        ...
        if (!empty($params['P_PriceMin'])) {
            ...
            $demo_data['Properties'] = array_filter($demo_data['Properties'], function($prop) use ($min_price) {
        ...
        if (!empty($params['P_PriceMax'])) {
            ...
            $demo_data['Properties'] = array_filter($demo_data['Properties'], function($prop) use ($max_price) {
                return $prop['Price'] <= $max_price;
            });
        }
        
        // Reindexar el array después de los filtros
        $demo_data['Properties'] = array_values($demo_data['Properties']);
        $demo_data['QueryInfo']['TotalCount'] = count($demo_data['Properties']);
